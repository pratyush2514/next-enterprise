...
type RequestAuthFn = (track: ITunesResult) => void

const AuthOverlayContext = createContext<RequestAuthFn | null>(null)
...
function SongsShellInner({ children }: { children: React.ReactNode }) {
  ...
  const [authTrack, setAuthTrack] = useState<ITunesResult | null>(null)
  ...
  const requestAuth = useCallback((track: ITunesResult) => {
    setAuthTrack(track)
  }, [])

  return (
    <AuthOverlayContext.Provider value={requestAuth}>
      <SongsPlaybackContext.Provider value={updateCurrentTrack}>
        ...
        {/* Auth overlay — rendered outside layout div for z-index isolation */}
        <AuthOverlay track={authTrack} onClose={() => setAuthTrack(null)} />
